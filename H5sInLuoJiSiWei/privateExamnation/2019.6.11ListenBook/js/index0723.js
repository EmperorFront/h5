...
var first= [
  "https://emperorfront.github.io/h5/H5sInLuoJiSiWei/privateExamnation/2019.6.11ListenBook/others/gomark.png",
...
function anmt() {//百分比和首屏logo动画
  ...
  var data = [];//数据数组
  var nub = 0;//当前加载数
  for (var s in others) {//others，所有的图片数组
    data = data.concat(others[s]);//concat，数组链接。 把img的数据模块挨个放在data数组。二维数组变一维数组。
  }
  for (var i = 0; i < data.length; i++) {
    var img = new Image();
    img.src = data[i];//预加载图片
